...
secondary gain and then a natural integer strictly above that maximum.  No
polytope library, homogeneity, balance relation, or HC4 state is involved.
-/
...
directly in `S`.  The scale is deliberately existential: later callers may
increase it further to meet nonnegativity or Hessian-clock constraints. -/
