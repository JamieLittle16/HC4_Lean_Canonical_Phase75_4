import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrVGreaterOnePlanarFirstLayer
import Mathlib.Tactic

/-!
# A19 locked special fibre of the singular planar-contact Rees

The auxiliary reverse Rees of the singular planar carrier is filtered by the
honest integral contact weight.  Its zero layer must therefore be the locked
pair-degree-one fibre.  The whole-carrier quotient rule makes that fibre
literal: it consists exactly of the retained source facet/outside endpoints.

This file records both the support equality and the exact two-monomial
polynomial equality.  The coefficients are unchanged coefficients of
`P.carrier`; no normalization or division is performed.
-/
...
/-- The locked facet endpoint has reverse order zero in the planar-contact
family. -/
theorem locked_facet_reverseOrder_eq_zero
    ...
    T.topFace.degree -
        Finsupp.weight (qsIntegralContactWeight (F.V + 1))
          C.ray.facetExponent = 0 := by
  rw [D.reverseOrder_eq_quotientContactOrder hthree houtThree]
...
theorem locked_outside_reverseOrder_eq_zero
  ...
  rw [← D.reverseOrder_eq_quotientContactOrder hthree houtThree,
    ← D.reverseOrder_eq_quotientContactOrder hthree houtThree] at hq
  rw [D.locked_facet_reverseOrder_eq_zero hthree houtThree] at hq
  exact hq

/-- The exact zero parameter layer consists only of the two locked source
endpoints. -/
...
/-- Coefficients on the zero layer are literally the original carrier
coefficients at the two locked endpoints. -/
...
/-- **Locked special fibre.**  The singular planar-contact Rees starts at the
literal two-monomial locked source polynomial. -/
