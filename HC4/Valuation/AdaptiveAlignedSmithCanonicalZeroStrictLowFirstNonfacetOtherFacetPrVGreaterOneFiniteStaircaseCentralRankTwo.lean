...
/-!
# Rank-two geometry at a central finite-staircase point

In the left `(1,V)`, `V>1` source-honest planar carrier, a support exponent
with both transverse source deficits zero is not an unresolved interior
configuration.  The exact staircase chord equation makes its longitudinal
coordinate the unique coordinate-`0` maximum of the whole carrier.

Hence the coordinate-`0` maximal initial form is literally that one source
monomial.  Its remaining coordinates `0` and `3` are both positive, so its
principal `(0,3)` Hessian minor is nonzero.  This produces concrete rank-two
Hessian geometry on an honest singular initial face; no three-term
subpolynomial is declared singular and no auxiliary Rees clock is introduced.
-/
...
/-- **A central roof-intersection point exposes literal rank-two geometry.**

The returned `CoordinateMaxInitialData` is the honest coordinate-`0` initial
face of `P.carrier`.  It is exactly the central source monomial, remains
Hessian-singular, and has a nonzero principal `(0,3)` Hessian minor. -/
theorem QsOtherFacetPrLeftVContactFrontierData.central_coordinateMax_face_rankTwo
  ...
  let D := HC4.Newton.coordinateMaxInitialData
    P.carrier hPne (0 : Fin 4)
  ...
  have hunique0 :
      ∀ e ∈ P.carrier.support, e 0 = c 0 → e = c := by
    intro e he he0
    have heChord := F.support_deficit_chord hthree houtThree he
    rw [he0] at heChord
    have hsum :
    ...
    exact F.support_eq_of_deficits_eq hthree houtThree he hc
      (by simpa [hc1] using he1)
      (by simpa [hc2] using he2)
