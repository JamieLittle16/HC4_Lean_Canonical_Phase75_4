...
/-!
# A19 lower pure unit one-fibre diagonal is impossible

On `j+2=k`, the common unit profile has degree exactly `k-1`.  The reflected
locked-based three-layer pencil and generic top second-variation identity then
produce a manifestly nonzero scalar product which singularity forces to zero.
-/
...
/-- Lower unit one-fibre elimination. -/
theorem unitLeft_oneFiber_lower_diagonal_impossible
  ...
  let A := lockedBinomialMomentHessian 1 F.locked.ell a b
  let B := parallelStaircaseMomentHessian 1 Alo.k Alo.j Alo.coefficientProfile
  let Cend := highestBinomialMomentHessian 1 F.highest.n
  ...
  have hqN : q < N := by dsimp [q, N]; omega
  have hm : 1 < Alo.k - 1 := by omega

  have hrefDet := D.det_unitLeftOneFiberReflectedThreeLayerMomentPencil_eq_zero
  ...
  have hdet :
    ...
    simpa [A, B, Cend, q, N, parameterThreeLayerMatrix,
      unitLeftOneFiberReflectedThreeLayerMomentPencil] using hrefDet
  ...
  have hTopMatrix :
      secondVariationTopMatrix A B (Alo.k - 1) =
        lockedLowerPureModeSecondJet
          (1 : K) (F.locked.ell : K) (Alo.k : K) b u := by
    apply Matrix.ext
    intro i j
