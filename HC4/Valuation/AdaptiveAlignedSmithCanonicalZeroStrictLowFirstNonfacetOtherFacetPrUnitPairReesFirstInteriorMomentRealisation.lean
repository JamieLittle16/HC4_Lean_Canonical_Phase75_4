...
/-!
# A19 moment realisation of the first positive unit pair-Rees interior layer

The selected first positive unit pair-Rees layer is a literal affine staircase
fibre with injective coordinate-zero parameter and unchanged source-carrier
coefficients.  This reconstructs that exact layer as `RankThreeAffineLineData`
and identifies its specialised Euler-scaled Hessian with the unit
`parallelStaircaseMomentHessian`.
-/
...
exponent of the selected unit pair-Rees layer. -/
...
/-- Exact affine-line realisation of the selected unit pair-Rees layer. -/
...
/-- The affine-line polynomial is literally the selected unit pair-Rees layer. -/
...
/-- Exact first positive unit pair-Rees moment identification. -/
