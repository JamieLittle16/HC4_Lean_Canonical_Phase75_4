import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrVGreaterOneFiniteStaircaseCrossRoofAffineCoordinates
import Mathlib.Tactic

/-!
# Exact affine interpolation on the exposed cross-roof face

The lower-hull face is one-dimensional in the literal source coordinates.
The cost equality first gives the exact transverse interpolation relation.
The already-verified deficit chord then determines coordinate `0`, and the
staircase monomial-curve equation determines coordinate `3`.

These are the final source-coordinate identities needed to package the face in
both orientations as `RankThreeAffineLineData`.
-/
...
/-- The staircase second equation in literal source coordinates. -/
