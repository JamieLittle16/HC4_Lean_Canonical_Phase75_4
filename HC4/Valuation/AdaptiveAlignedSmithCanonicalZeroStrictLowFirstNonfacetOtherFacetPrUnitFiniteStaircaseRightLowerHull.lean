...
/-!
# Source-honest lower hull of the mirrored unit finite staircase

Project the actual right unit carrier to source deficits `(e₁,e₃)`.  The
locked outside endpoint lies on `e₁=0`, while the primitive highest endpoint
lies on `e₃=0`; the finite maximal-slope selector gives the exact mirror of the
left unit lower-hull alternative.
-/
...
structure QsOtherFacetPrUnitRightLowerHullData
...
/-- Either the right unit source carrier contains a central `e₁=e₃=0` point,
or it has an honest positive lower-hull wall leaving the `e₁=0` roof. -/
theorem QsOtherFacetPrUnitRightContactFrontierData.central_or_lowerHull
    {C : AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetCrossFacetData T .qs}
    {P : QsOtherFacetPlanarCarrierPackage C .pr}
    {S : QsOtherFacetPlanarHighestPairSlicePackage C .pr P}
    {R : QsOtherFacetContactQuadraticReesPackage C}
    ...
    (∃ e ∈ P.carrier.support, e 1 = 0 ∧ e 3 = 0) ∨
      Nonempty (QsOtherFacetPrUnitRightLowerHullData F) := by
  classical
