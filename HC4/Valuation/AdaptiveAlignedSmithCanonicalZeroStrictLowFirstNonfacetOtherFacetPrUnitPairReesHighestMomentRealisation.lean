...
/-!
# A19 unit highest pair-Rees special fibre as the primitive highest moment Hessian

The unit pair-degree reverse Rees is oriented from the primitive highest slice.
Its zero layer is the literal source-honest highest binomial.  This file
identifies the specialised Euler-scaled Hessian of that exact zero layer with
the `V = 1` highest-binomial moment Hessian.
-/
...
/-- Exact highest zero-layer moment identification in the left unit
orientation.  The coefficients are literally those of the retained highest
slice. -/
theorem zeroLayer_specialisedEulerHessian_eq_highestBinomialMomentHessian_unitLeft
      ...
      · funext i
        fin_cases i <;>
          simp [exponent, e1,
            ...
            F.highest_V_eq_one, hcast_n_sub_one] <;> ring
  }
