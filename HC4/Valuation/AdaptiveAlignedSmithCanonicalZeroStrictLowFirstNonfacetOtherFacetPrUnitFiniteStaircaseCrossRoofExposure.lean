...
/-!
# The unit source lower hull reaches the opposite roof

This is the source-honest unit analogue of the verified nonunit cross-roof
exposure.  The lower-hull wall is turned into an exact singular initial face;
if that face missed `e₂=0`, a coordinate-max refinement would isolate a
monomial with all four coordinates positive, contradicting its Hessian
determinant.
-/
...
theorem weight_bound (D : QsOtherFacetPrUnitLeftLowerHullData F) :
...
theorem lo_weight_eq (D : QsOtherFacetPrUnitLeftLowerHullData F) :
...
theorem edge_weight_eq (D : QsOtherFacetPrUnitLeftLowerHullData F) :
...
theorem lo_mem_face (D : QsOtherFacetPrUnitLeftLowerHullData F) :
...
theorem edge_mem_face (D : QsOtherFacetPrUnitLeftLowerHullData F) :
...
theorem face_support_subset (D : QsOtherFacetPrUnitLeftLowerHullData F) :
...
theorem face_hessian_zero (D : QsOtherFacetPrUnitLeftLowerHullData F) :
...
/-- **The exact unit lower-hull face meets the opposite `e₂=0` roof.** -/
