...
/-!
# A19 moment realisation of the locked planar-contact special fibre

The zero parameter layer is already proved to be the literal locked source
pair.  This file packages that pair as a degree-one affine rank-three line and
identifies its specialised Euler-scaled Hessian with
`lockedBinomialMomentHessian`, retaining the original carrier coefficients.
-/
...
/-- **Exact locked zero-layer moment identification.**  The two coefficients
are the literal facet/outside coefficients of the planar carrier. -/
theorem zeroLayer_specialisedEulerHessian_eq_lockedBinomialMomentHessian
    ...
    (fun r s =>
      ...
      HC4.Polynomial.lockedBinomialMomentHessian F.V F.locked.ell a b := by
  classical
