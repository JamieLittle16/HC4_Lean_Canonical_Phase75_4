...
import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrVGreaterOneFiniteStaircaseOneFiberMiddleImpossible

/-!
# Complete A19 one-fibre elimination

The common-profile degree coupling leaves exactly three adjacent staircase
diagonals.  The lower and upper pure diagonals were already eliminated by the
second-variation argument; the middle diagonal is now eliminated directly by
the locked-to-highest intermediate endpoint variation.  Hence coincident low
and high strict-interior extrema are impossible.
-/
...
/-- **Complete one-fibre contradiction.** -/
theorem oneFiber_impossible
  ...
  rcases F.oneFiber_three_diagonals Alo Ahi hthree houtThree hext with
    hlow | hmiddle | hupp
  · exact D.oneFiber_lower_diagonal_impossible
      F Alo Ahi hthree houtThree hnot hext hlow
  · exact D.oneFiber_middle_impossible
      F Alo Ahi hthree houtThree hnot hext
  · exact D.oneFiber_upper_diagonal_impossible
