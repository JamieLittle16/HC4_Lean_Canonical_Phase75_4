...
/-!
# Rank-two geometry at a mirrored central finite-staircase point

For the right `(V,1)`, `V>1` orientation, a source support point with
`e₁=e₃=0` is the transverse-coordinate mirror of the left central point.  The
right deficit chord makes it the unique coordinate-`0` maximum.  Its exact
initial face is therefore one source monomial, and the right staircase curve
forces coordinate `2` to be positive.  Hence the honest initial face carries a
nonzero principal `(0,2)` Hessian minor.
-/
...
/-- A mirrored central roof-intersection point exposes literal rank-two
geometry on the honest coordinate-`0` initial face. -/
theorem QsOtherFacetPrRightVContactFrontierData.central_coordinateMax_face_rankTwo
  ...
  have hmax0 : ∀ e ∈ P.carrier.support, e 0 ≤ c 0 := by
    ...
    have he3Z : (0 : ℤ) ≤ (e 3 : ℤ) := by positivity
    have hterm1 :
        (0 : ℤ) ≤ (F.locked.ell : ℤ) * (e 1 : ℤ) :=
      mul_nonneg (le_of_lt hellZ) he1Z
    have hterm3 :
        (0 : ℤ) ≤ ((F.highest.n : ℤ) - 1) * (e 3 : ℤ) :=
      mul_nonneg (le_of_lt hnOneZ) he3Z
    have hleft :
        (0 : ℤ) ≤
          (F.locked.ell : ℤ) * (e 1 : ℤ) +
            ((F.highest.n : ℤ) - 1) * (e 3 : ℤ) := by
      nlinarith only [hterm1, hterm3]
    have hdiff :
