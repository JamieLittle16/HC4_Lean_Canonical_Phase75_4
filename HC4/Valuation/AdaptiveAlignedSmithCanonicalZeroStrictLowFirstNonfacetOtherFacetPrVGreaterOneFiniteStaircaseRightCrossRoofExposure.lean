import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrVGreaterOneFiniteStaircaseRightLowerHull
import HC4.Newton.FiniteSupportExposedVertex
import HC4.Polynomial.MonomialHessian
import Mathlib.Tactic

/-!
# The mirrored source lower hull reaches the opposite roof

The right lower-hull wall lives in source deficits `(e₁,e₃)`.  Its exact
singular initial face must meet the opposite `e₃=0` roof.  Otherwise maximizing
`e₁` makes a singleton source monomial; the hull arithmetic forces `e₀>0` and
the right staircase curve forces `e₂>0`, so all four exponents are positive,
contradicting the monomial Hessian obstruction.
-/
...
/-- Positive coefficient of source coordinate `1` in the mirrored hull cost. -/
def gap (D : QsOtherFacetPrRightVLowerHullData F) : ℕ :=
  D.lo 3 - D.edge 3

/-- Integer source weight whose maximal face is the mirrored lower-hull wall. -/
def weight (D : QsOtherFacetPrRightVLowerHullData F) : Fin 4 → ℤ :=
  fun i => if i = (1 : Fin 4) then -(D.gap : ℤ)
    else if i = (3 : Fin 4) then -(D.edge 1 : ℤ) else 0

/-- Exact level of the mirrored lower-hull wall. -/
def level (D : QsOtherFacetPrRightVLowerHullData F) : ℤ :=
  -(D.edge 1 : ℤ) * (D.lo 3 : ℤ)

/-- Actual source initial form cut out by the mirrored lower-hull wall. -/
def face (D : QsOtherFacetPrRightVLowerHullData F) :
    MvPolynomial (Fin 4) K :=
  HC4.Polynomial.initialForm D.weight D.level P.carrier

@[simp] theorem gap_pos (D : QsOtherFacetPrRightVLowerHullData F) :
    0 < D.gap := by
  exact Nat.sub_pos_of_lt D.edge_three_lt

/-- Evaluation of the mirrored lower-hull weight. -/
theorem weight_eq (D : QsOtherFacetPrRightVLowerHullData F)
    (e : Fin 4 →₀ ℕ) :
...
/-- The selected mirrored wall is maximal for the source carrier. -/
theorem weight_bound (D : QsOtherFacetPrRightVLowerHullData F) :
...
/-- The lower roof point survives in the exact mirrored initial face. -/
theorem lo_weight_eq (D : QsOtherFacetPrRightVLowerHullData F) :
...
/-- The selected leaving point lies on the same exact face. -/
theorem edge_weight_eq (D : QsOtherFacetPrRightVLowerHullData F) :
...
/-- Lower endpoint membership in the exact mirrored face. -/
theorem lo_mem_face (D : QsOtherFacetPrRightVLowerHullData F) :
...
/-- Mirrored face support remains literal source support. -/
theorem face_support_subset (D : QsOtherFacetPrRightVLowerHullData F) :
    D.face.support ⊆ P.carrier.support := by
  dsimp [face]
  exact HC4.Polynomial.support_initialForm_subset D.weight D.level P.carrier

/-- Every point of the exact mirrored face satisfies the hull cost equality. -/
...
/-- Hessian singularity passes to the exact mirrored hull face. -/
theorem face_hessian_zero (D : QsOtherFacetPrRightVLowerHullData F) :
...
/-- The exact mirrored lower-hull face meets the opposite `e₃=0` roof. -/
...
/-- Source-facing form of the mirrored opposite-roof point. -/
