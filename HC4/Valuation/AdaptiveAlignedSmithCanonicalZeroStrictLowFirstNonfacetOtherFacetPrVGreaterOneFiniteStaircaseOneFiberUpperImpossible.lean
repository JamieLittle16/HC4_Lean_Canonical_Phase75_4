import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrVGreaterOneFiniteStaircaseOneFiberSecondJet
...
/-!
# A19 upper pure one-fibre diagonal is impossible

Assume the unique strict-interior fibre lies on the upper diagonal `j=k`.
The dual endpoint Euler equations force its honest longitudinal profile to have
ordinary degree exactly `k`, with nonzero top coefficient.

Apply the second parameter-gap jet to the exact singular three-layer pair-Rees
pencil.  At longitudinal degree `2+2k`, the generic no-cancellation theorem
sees only the leading primitive-highest endpoint matrix and two copies of the
leading interior matrix.  Any resonant locked layer has degree at most one and
cannot contribute at that degree.

The resulting numerical second variation is

  2 V(V+1)(n-1)^2 k^2 (n-2) d^2 u^2,

and every factor is nonzero in the live strict-interior branch.  Contradiction.
-/
...
/-- **Upper one-fibre elimination.** -/
theorem oneFiber_upper_diagonal_impossible
  ...
  have hgap : ∀ i j, HasNoPositiveParameterCoeffBelow q (M i j) := by
    intro i j
    simpa [q, M] using oneFiberThreeLayerMomentPencil_hasGap F Alo i j
  ...
  have hjet := matrixParameterGapSecondJet_oneFiberThreeLayer
    (K := K) F Alo
  ...
  have hCdeg : ∀ i j, (C2 i j).natDegree ≤ 1 := by
    ...
    · have hCeq :
        ...
        dsimp [C2, oneFiberSecondOrderMomentMatrix, M, q] at *
        simp [oneFiberThreeLayerMomentPencil, h2q0, h2qq, hres]
      rw [hCeq]
  ...
  have hTopMatrix :
      secondVariationTopMatrix A B Alo.k =
        highestUpperPureModeSecondJet
          (F.V : K) (F.highest.n : K) (Alo.k : K) d u := by
    apply Matrix.ext
    intro i j
