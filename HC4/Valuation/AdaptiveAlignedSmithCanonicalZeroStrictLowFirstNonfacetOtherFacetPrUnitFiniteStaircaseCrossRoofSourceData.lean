import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrUnitFiniteStaircaseCrossRoofExposure
import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrUnitStaircaseClassification
import Mathlib.Tactic

/-!
# Source-honest exposed cross-roof endpoint data for the unit staircase

The exact unit lower-hull face now contains one literal source monomial on
each transverse coordinate roof.  This file packages those endpoints in the
same staircase variables used by the verified state-free cross-roof terminal
arithmetic, specialized honestly to quotient parameter `V = 1`.

No support is reconstructed: both endpoints remain actual monomials of one
exact singular initial face of the source carrier.
-/
...
/-- Literal endpoint package for one exposed unit cross-roof face. -/
structure QsOtherFacetPrUnitLeftExposedCrossRoofData
  ...
  hull : QsOtherFacetPrUnitLeftLowerHullData F
  hi : Fin 4 →₀ ℕ
...
namespace QsOtherFacetPrUnitLeftLowerHullData
...
/-- Package the two literal roof endpoints of the exact unit lower-hull face
in finite-staircase coordinates. -/
theorem exposedCrossRoofData
    (D : QsOtherFacetPrUnitLeftLowerHullData F)
    (hthree : MvRankThreeOnFacet .qs C.ray.facetExponent)
    (houtThree : MvRankThreeOnFacet .pr C.ray.outsideExponent) :
    Nonempty (QsOtherFacetPrUnitLeftExposedCrossRoofData F) := by
  classical
  ...
  have hloThree : D.lo 3 = jLo := by
    ...
    norm_num at hs
    have hthreeZ : (D.lo 3 : ℤ) = (jLo : ℤ) := by
      linear_combination hs + hfirstZ
    exact_mod_cast hthreeZ

  have hhiThree : hi 3 = kHi - 1 := by
    ...
    rw [hi2] at hs
    norm_num at hs
    have hthreeZ : (hi 3 : ℤ) = ((kHi - 1 : ℕ) : ℤ) := by
      rw [hsubCast]
      linear_combination hs - hkHiZ
    exact_mod_cast hthreeZ
...
end QsOtherFacetPrUnitLeftLowerHullData

/-- Whole-carrier unit transition: either a central roof-intersection monomial
is present, or the actual source has an exposed cross-roof face with fully
packaged staircase endpoints. -/
theorem QsOtherFacetPrUnitLeftContactFrontierData.central_or_exposedCrossRoof
    {C : AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetCrossFacetData
      T .qs}
    {P : QsOtherFacetPlanarCarrierPackage C .pr}
    {S : QsOtherFacetPlanarHighestPairSlicePackage C .pr P}
