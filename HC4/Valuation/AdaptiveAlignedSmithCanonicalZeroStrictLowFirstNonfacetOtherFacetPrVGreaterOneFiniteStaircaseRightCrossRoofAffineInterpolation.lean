import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrVGreaterOneFiniteStaircaseRightCrossRoofAffineCoordinates
import Mathlib.Tactic

/-!
# Exact affine interpolation on the mirrored exposed cross-roof face

The mirrored lower-hull face is one-dimensional in the literal source
coordinates.  The cost equality gives the transverse relation in `(e₁,e₃)`,
the verified source-deficit chord determines coordinate `0`, and the right
staircase curve determines coordinate `2`.
-/
...
/-- The mirrored lower-hull cost equation normalized by its two roof endpoints. -/
...
/-- The right staircase second equation in literal source coordinates. -/
...
/-- Coordinate `0` interpolates affinely between the literal roof endpoints,
with coordinate `1` as parameter. -/
...
/-- Coordinate `2` interpolates affinely between the same literal roof endpoints. -/
