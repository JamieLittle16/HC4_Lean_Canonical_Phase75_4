import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrUnitFiniteStaircaseRightCrossRoofExposure
import HC4.Valuation.AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetOtherFacetPrUnitStaircaseClassification
import Mathlib.Tactic

/-!
# Source-honest mirrored exposed cross-roof endpoint data for the unit case

The mirrored unit lower-hull face contains literal source points on `e₁=0`
and `e₃=0`.  We package them in the staircase integers
`(kLo,jLo,kHi,jHi,q,v)` used by the state-free mirror terminal arithmetic.
-/
...
structure QsOtherFacetPrUnitRightExposedCrossRoofData
  ...
  hull : QsOtherFacetPrUnitRightLowerHullData F
  hi : Fin 4 →₀ ℕ
...
theorem exposedCrossRoofData
    ...
    Nonempty (QsOtherFacetPrUnitRightExposedCrossRoofData F) := by
  classical
  rcases D.exists_wRoof_on_face hthree houtThree with ⟨hi, hiFace, hi3⟩
  ...
  have hloSecondNat : D.lo 0 + D.lo 3 = jLo + 1 := by
    simpa [HC4.Polynomial.rankThreeQuotientCoordinate, D.lo_one_zero] using hloSecond
  ...
  have hkLoPos : 0 < kLo := by
    ...
    dsimp [kLo]
    simpa [HC4.Polynomial.rankThreeQuotientCoordinate, D.lo_one_zero] using hp
  have hqPos : 0 < q := by simpa [q] using D.lo_three_pos
  ...
  have hwallLo :
    ...
    have hsecondSumZ : (D.lo 0 : ℤ) + (D.lo 3 : ℤ) = (jLo : ℤ) + 1 := by
      exact_mod_cast hloSecondNat
    have hsecondZ : (D.lo 0 : ℤ) + (D.lo 3 : ℤ) - 1 = (jLo : ℤ) := by
  ...
  have hwallHi :
    ...
    have hhi0Z : (hi 0 : ℤ) = (jHi : ℤ) + 1 := by exact_mod_cast hhiSecondNat
    have hkHiZ : (kHi : ℤ) = (hi 0 : ℤ) + (hi 1 : ℤ) := by dsimp [kHi]
    have hleft : (hi 0 : ℤ) - 1 = (jHi : ℤ) := by linarith only [hhi0Z]
    have hright :
  ...
  have hklt : kLo < kHi :=
    HC4.Polynomial.crossRoof_pair_lt_of_roof_signs
      F.highest.n_two_le F.locked.ell_pos hwallLo hwallHi hloRoof hhiRoof
  ...
  have hloTwo : D.lo 2 = jLo := by
    ...
    simp only [HC4.Polynomial.rankThreeQuotientCoordinate_firstTransverse,
      ...
      one_mul] at hs
    push_cast at hs
    have hsecondZ : (D.lo 0 : ℤ) + (D.lo 3 : ℤ) = (jLo : ℤ) + 1 := by
      exact_mod_cast hloSecondNat
    rw [D.lo_one_zero] at hs
    norm_num at hs
    have htwoZ : (D.lo 2 : ℤ) = (jLo : ℤ) := by
      linear_combination hs + hsecondZ
    exact_mod_cast htwoZ

  have hhiTwo : hi 2 = kHi - 1 := by
    ...
    rw [hi3] at hs
    norm_num at hs
    have htwoZ : (hi 2 : ℤ) = ((kHi - 1 : ℕ) : ℤ) := by
      rw [hsubCast]
...
/-- Whole-carrier mirrored unit transition. -/
theorem QsOtherFacetPrUnitRightContactFrontierData.central_or_exposedCrossRoof
    {C : AdaptiveAlignedSmithCanonicalZeroStrictLowFirstNonfacetCrossFacetData T .qs}
    {P : QsOtherFacetPlanarCarrierPackage C .pr}
    {S : QsOtherFacetPlanarHighestPairSlicePackage C .pr P}
    {R : QsOtherFacetContactQuadraticReesPackage C}
