...
/-- Coordinate `0` is injective on the selected unit pair-Rees layer. -/
theorem eq_of_zeroCoordinate_eq
  ...
  · have hsum : e 0 + e 2 = e 0 + f 2 := by
    ...
    exact Nat.add_left_cancel hsum
  · have hsum : e 0 + e 3 = e 0 + f 3 := by
      calc
        e 0 + e 3 = A.k + A.j := he3
        _ = f 0 + f 3 := hf3.symm
        _ = e 0 + f 3 := by rw [hzero]
    exact Nat.add_left_cancel hsum

/-- Honest one-variable coefficient profile of the selected unit pair-Rees layer. -/
noncomputable def coefficientProfile
    ...
    {F : QsOtherFacetPrUnitLeftContactFrontierData C P S R}
    {D : QsOtherFacetPrPairReesData C P S F.highest.n}
    (_A : QsOtherFacetPrUnitLeftPairFirstInteriorAffineLayerData F D) : Polynomial K :=
  let L := familyParameterLayer D.family
    (firstPositiveActualParameterOrder D.family D.positiveLayer)
  ∑ e ∈ L.support, Polynomial.monomial (e 0) (MvPolynomial.coeff e L)

/-- The profile retains each selected unit-layer coefficient literally. -/
...
/-- The extracted first unit pair-Rees profile is nonzero. -/
